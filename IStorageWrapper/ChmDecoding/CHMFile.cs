...
namespace HtmlHelp.ChmDecoding
{
    ...
    public sealed class CHMFile : IDisposable
    {
        ...
        public CHMFile(HtmlHelpSystem systemInstance, string chmFile)
            : this(systemInstance, chmFile, false, null)
        {
        }
        ...
        public CHMFile(HtmlHelpSystem systemInstance, string chmFile, DumpingInfo dmpInfo)
            : this(systemInstance, chmFile, false, dmpInfo)
        {
        }
        ...
        internal CHMFile(HtmlHelpSystem systemInstance, string chmFile, bool onlySystemData, DumpingInfo dmpInfo)
        {
            ...
            if (File.Exists(chmFile))
            {
                ...
                DateTime dtEndHH = DateTime.Now;
                TimeSpan elapsedHH = dtEndHH - dtStartHH;
                Debug.WriteLine("--- HtmlHelp file read in " + elapsedHH.ToString());
                Debug.Unindent();
                ...
                if (_mustWriteDump)
                {
                    ...
                    Debug.Indent();
                    DateTime dtStartD = DateTime.Now;

                    _mustWriteDump = !SaveDump(dmpInfo);

                    DateTime dtEndD = DateTime.Now;
                    TimeSpan elapsedD = dtEndD - dtStartD;
                    Debug.WriteLine("--- Dump written in " + elapsedD.ToString());
                    Debug.Unindent();
                }
                ...
                if (_systemFile.DefaultTopic.Length > 0)
                {
                    ...
                    iw = new ITStorageWrapper(chmFile, false);
                    _currentWrapper = iw;

                    // tryo to open the topic file
                    fileObject = iw.OpenUCOMStream(null, _systemFile.DefaultTopic);
                    if (fileObject != null)
                    {
        ...
        private void ReadFile(string fname, HtmlHelpFileType type)
        {
            ...
            {
                fileObject = iw.OpenUCOMStream(null, "#SYSTEM");
                if (fileObject != null)
                {
                    ...
                    fileObject.Close();
                    _systemFile = new CHMSystem(fileData, this);
                    DateTime dtEnd = DateTime.Now;
                    TimeSpan elapsed = dtEnd - dtStart;
                    Debug.WriteLine("time elapsed - " + elapsed.ToString());
                    Debug.Unindent();
                }

                fileObject = iw.OpenUCOMStream(null, "#IDXHDR");
                if (fileObject != null)
                {
                    ...
                    fileObject.Read(fileData, 0, (int)fileObject.Length);
                    fileObject.Close();
                    _idxhdrFile = new CHMIdxhdr(fileData, this);
                    DateTime dtEnd = DateTime.Now;
                    TimeSpan elapsed = dtEnd - dtStart;
                    Debug.WriteLine("time elapsed - " + elapsed.ToString());
                    Debug.Unindent();
                ...
                if ((!_dumpRead) && (CheckDump(_dmpInfo)) && (_dumpReadTrys == 0))
                {
                    ...
                    DateTime dtEndD = DateTime.Now;
                    TimeSpan elapsedD = dtEndD - dtStartD;
                    Debug.WriteLine("--- Dump read in " + elapsedD.ToString());
                    Debug.Unindent();
                }

                if ((!_dumpRead) || (!_dmpInfo.DumpStrings))
                {
                    fileObject = iw.OpenUCOMStream(null, "#STRINGS");
                    if (fileObject != null)
                    {
                        ...
                        fileObject.Read(fileData, 0, (int)fileObject.Length);
                        fileObject.Close();
                        _stringsFile = new CHMStrings(fileData, this);
                        DateTime dtEnd = DateTime.Now;
                        TimeSpan elapsed = dtEnd - dtStart;
                        Debug.WriteLine("time elapsed - " + elapsed.ToString());
                        Debug.Unindent();
                ...
                {
                    fileObject = iw.OpenUCOMStream(null, "#URLSTR");
                    if (fileObject != null)
                    {
                        ...
                        fileObject.Read(fileData, 0, (int)fileObject.Length);
                        fileObject.Close();
                        _urlstrFile = new CHMUrlstr(fileData, this);
                        DateTime dtEnd = DateTime.Now;
                        TimeSpan elapsed = dtEnd - dtStart;
                        Debug.WriteLine("time elapsed - " + elapsed.ToString());
                        Debug.Unindent();
                ...
                {
                    fileObject = iw.OpenUCOMStream(null, "#URLTBL");
                    if (fileObject != null)
                    {
                        ...
                        fileObject.Read(fileData, 0, (int)fileObject.Length);
                        fileObject.Close();
                        _urltblFile = new CHMUrltable(fileData, this);
                        DateTime dtEnd = DateTime.Now;
                        TimeSpan elapsed = dtEnd - dtStart;
                        Debug.WriteLine("time elapsed - " + elapsed.ToString());
                        Debug.Unindent();
                ...
                {
                    fileObject = iw.OpenUCOMStream(null, "#TOPICS");
                    if (fileObject != null)
                    {
                        ...
                        fileObject.Read(fileData, 0, (int)fileObject.Length);
                        fileObject.Close();
                        _topicsFile = new CHMTopics(fileData, this);
                        DateTime dtEnd = DateTime.Now;
                        TimeSpan elapsed = dtEnd - dtStart;
                        Debug.WriteLine("time elapsed - " + elapsed.ToString());
                        Debug.Unindent();
                ...
                if (!_onlySystem)
                {
                    ...
                    {
                        fileObject = iw.OpenUCOMStream(null, "#TOCIDX");
                        if ((fileObject != null) && (_systemFile.BinaryTOC))
                        {
                            ...
                            fileObject.Read(fileData, 0, (int)fileObject.Length);
                            fileObject.Close();
                            _tocidxFile = new CHMTocidx(fileData, this);
                ...
                if ((_systemFile != null) && (!_onlySystem))
                {
                    if (!_systemFile.BinaryTOC)
                    {
                        ...
                        {
                            fileObject = iw.OpenUCOMStream(null, _systemFile.ContentsFile);
                            if (fileObject != null)
                            {
            ...
            if (type != HtmlHelpFileType.CHQ) // no index information in CHQ files (only fulltext search)
            {
                if ((_systemFile != null) && (!_onlySystem))
                {
                    if (!_systemFile.BinaryIndex)
                    {
                        ...
                        {
                            fileObject = iw.OpenUCOMStream(null, _systemFile.IndexFile);
                            if (fileObject != null)
                            {
                    ...
                    else
                    {
                        ...
                        {
                            Interop.IStorage storage = iw.OpenSubStorage(null, "$WWKeywordLinks");

                            if (storage != null)
                            {
                                fileObject = iw.OpenUCOMStream(storage, "BTree");
                                if (fileObject != null)
                                {
                                    ...
                                    fileObject.Read(fileData, 0, (int)fileObject.Length);
                                    fileObject.Close();
                                    _kLinks = new CHMBtree(fileData, this);
                            ...
                            storage = iw.OpenSubStorage(null, "$WWAssociativeLinks");
                            ...
                            if (storage != null)
                            {
                                ...
                                if (fileObject != null)
                                {
                                    ...
                                    fileObject.Read(fileData, 0, (int)fileObject.Length);
                                    fileObject.Close();
                                    _aLinks = new CHMBtree(fileData, this);
            ...
            if ((type != HtmlHelpFileType.CHI) && (type != HtmlHelpFileType.CHW))
            {
                if ((_systemFile != null) && (!_onlySystem))
                {
                    if (_systemFile.FullTextSearch)
                    {
                        ...
                        {
                            fileObject = iw.OpenUCOMStream(null, "$FIftiMain");
                            if (fileObject != null)
                            {
                                ...
                                fileObject.Read(fileData, 0, (int)fileObject.Length);
                                fileObject.Close();
                                _ftSearcher = new FullTextEngine(fileData, this);
        ...
        internal ArrayList EnumFilesByExtension(string extension)
        {
            ...
            if (_currentWrapper == null)
                iw = new ITStorageWrapper(_chmFileName, false);
            else
                iw = _currentWrapper;

            iw.EnumIStorageObject();

            foreach (FileObject fileObject in iw.foCollection)
            {
                if (((fileObject.CanRead) && (fileObject.FileName.ToLower().EndsWith(extension))))
                {
                    arrRet.Add(fileObject.FileName);
                }
            }
        ...
        public ArrayList ParseHHC(string hhcFile)
        {
            ...
            fileObject = iw.OpenUCOMStream(null, hhcFile);
            if (fileObject != null)
            {
                Debug.WriteLine("Parsing HHC-toc: ");
                Debug.Indent();
        ...
        internal ITStorageWrapper CurrentStorageWrapper
        {
