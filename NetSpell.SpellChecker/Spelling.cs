...
//		private Regex _digitRegex = new Regex(@"^\d", RegexOptions.Compiled); // this is wrong for finding numbers
		...
		private void ReplaceChars(ref ArrayList tempSuggestion)
		{
			...
			for (int i = 0; i < replacementChars.Count; i++)
			{
				...
				while (pos > -1)
				{
					...
					// added if to catch some sort of bug
					if((pos + key.Length) < (CurrentWord.Length + 1) )
					{
		...
		/// </remarks>
		public string DeleteWord(StringBuilder inputText,bool calculateWords)
		{
			...
			if(calculateWords)
			{
				words = _wordEx.Matches(inputText.ToString());
				
				//find the index of the duplicate word
				for(int i =0;i< words.Count; i++)
				{
					if(words[i].Value.Trim().ToLower()  == replacedWord.Trim().ToLower())
					{
						if(words[i+1].Value.Trim().ToLower()  == replacedWord.Trim().ToLower())
						{
							replacedIndex = i+1;
							break;
						}
				...
				if(replacedIndex == 0)
				{
					TraceWriter.TraceWarning("Could not find a duplicate word!!");
					return inputText.ToString();				}
			}
			else
			{
				words = _words;
				replacedIndex = this.WordIndex;
			}
			...
			int index = words[replacedIndex].Index;
			int length = words[replacedIndex].Length;
			
			// adjust length to remove extra white space after first word
			
			if (index == 0
				&& index + length < inputText.Length 
				&& inputText[index+length] == ' ')
			{
			...
			// adjust index to remove extra white space before punctuation
				...
				&& index + length < inputText.Length 
				&& inputText[index-1] == ' ' 
				&& char.IsPunctuation(inputText[index+length]))
			{					
				index--;
				length++;
			}
			...
			string deletedWord = inputText.ToString(index, length);
		...
		public bool SpellCheck(int startWordIndex, int endWordIndex)
		{
			...
			for (int i = startWordIndex; i <= endWordIndex; i++) 
			{
				...
				if(CheckString(currentWord)) 
				{
					...
					else if(_words.Count != i && i > 0 && _words[i-1].Value.ToString() == currentWord 
		...
		/// <summary>
		/// Access the regular express that calulates the word list
		/// </summary>
		public Regex WordEx {
			get { return _wordEx; }
		}
        ...
        public bool SplitUnderScore { get; set; }
    } 
}
