		...
		private void LoadUserFile()
		{
			...
			if (File.Exists(filePath)) 
			{
				...
				while (sr.Peek() >= 0) 
				{
					...
					{
						if(!_userWords.ContainsKey(tempLine))
						{
							_userWords.Add(tempLine, tempLine);
						}
						else
						{
							TraceWriter.TraceInfo("Duplicate word found in User Dictionary; Word:{0}", tempLine);
						}
				...
				foreach (PhoneticRule rule in _phoneticRules)
				{
						...
						&& rule.ConditionCount <= tempWord.Length)
					{
						...
						for (int i = 0;  i < rule.ConditionCount; i++) 
						{
							...
							// check that the letter is within ascii chr boundary
							if(charCode >= rule.Condition.Length)
							{
								TraceWriter.TraceError("Character code is beyond rule.Condition boundary");
								break;
							}
